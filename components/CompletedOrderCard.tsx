                ...
                {isExpanded && (
                    <div className={`p-4 border-t ${order.isDeleted ? 'text-gray-500' : ''}`}>
                        ...
                        <div className="mt-4 pt-4 border-t flex flex-wrap justify-end gap-3">
                            ...
                            {onPrintKitchenOrder && (
                                <button onClick={() => onPrintKitchenOrder(order)} className="px-4 py-2 bg-gray-700 text-white text-base font-semibold rounded-md hover:bg-gray-600 flex items-center gap-2 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed border border-gray-600 shadow-sm" disabled={order.isDeleted}>
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                    พิมพ์ใบครัว
                                </button>
                            )}
