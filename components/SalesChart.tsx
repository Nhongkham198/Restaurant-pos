    ...
    return (
        <div>
            ...
            <div className="w-full border border-gray-200 rounded-lg bg-gray-50 overflow-x-auto">
                ...
                <div className="h-64 flex items-end gap-1 sm:gap-2 p-4 pt-10 min-w-[600px]">
                    {data.map((value, index) => (
                        <div key={index} className="flex-1 h-full flex flex-col items-center justify-end group relative">
                            <div 
                                className="w-full bg-blue-400 hover:bg-blue-600 rounded-t-md transition-all duration-300 ease-out min-w-[8px]"
                            ...
                            <span className="text-xs text-gray-500 mt-1 whitespace-nowrap">{labels[index]}</span>
                        </div>
                    ))}
                </div>
            </div>
