        ...
        <div className={cardClasses}>
            <header className={`p-4 flex justify-between items-center ${order.isDeleted ? 'bg-red-100/60' : 'bg-red-50'}`}>
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                        <div className="flex items-baseline gap-2 flex-wrap">
                            <p className={`font-bold text-xl ${order.isDeleted ? 'text-red-700 line-through' : 'text-red-700'}`}>
                ...
                <div className="flex items-center gap-4 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <p className={`text-2xl font-bold ${order.isDeleted ? 'text-red-700 line-through' : 'text-red-700'}`}>{totalValue.toLocaleString(undefined, { minimumFractionDigits: 2 })} ฿</p>
