...
const NavButton: React.FC<{
    ...
    isDark?: boolean;
}> = ({ label, isActive, onClick, icon, badge, bottomBadge, disabled = false, activeClassName, isDark = false }) => (
    <button
        onClick={onClick}
        disabled={disabled}
...
export const Header: React.FC<HeaderProps> = ({ 
    ...
    const isKitchenView = currentView === 'kitchen';

    return (
        <header className={`p-3 sticky top-0 z-40 hidden md:block transition-all duration-300 ${
            isKitchenView ? 'bg-gray-950 shadow-none border-b border-gray-800' : 'bg-white shadow-md'
        }`}>
            <div className="flex justify-between items-center max-w-screen-2xl mx-auto">
                <div className="flex items-center gap-3">
                    <div className="relative">
                        {logoUrl ? (
                            ...
                            <div className={`h-10 w-10 rounded-md flex items-center justify-center transition-colors ${
                                isKitchenView ? 'bg-gray-800' : 'bg-blue-100'
                            }`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${isKitchenView ? 'text-gray-400' : 'text-blue-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="hidden md:block">
                        {isEditMode && isAdmin ? (
                            <input type="text" value={restaurantName} onChange={(e) => onRestaurantNameChange(e.target.value)} className={`text-xl font-bold p-1 border-b-2 border-blue-500 focus:ring-0 focus:border-blue-600 rounded-none transition-colors ${
                                isKitchenView ? 'bg-gray-900 text-white' : 'bg-blue-50 text-gray-800'
                            }`}/>
                        ) : (
                            <h1 className={`text-xl font-bold transition-colors ${isKitchenView ? 'text-white' : 'text-gray-800'}`}>{restaurantName}</h1>
                        )}
                ...
                <nav className={`flex items-center gap-2 p-1.5 rounded-xl transition-all duration-300 ${
                    ...
                    <NavButton label="ประวัติ" isActive={currentView === 'history'} onClick={() => onViewChange('history')} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" /></svg>} activeClassName="bg-teal-500 hover:bg-teal-600 text-white shadow-md" isDark={isKitchenView} />
                    <NavButton label="สต็อก" isActive={currentView === 'stock'} onClick={() => onViewChange('stock')} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>} activeClassName="bg-cyan-600 hover:bg-cyan-700 text-white shadow-md" badge={stockBadgeCount} isDark={isKitchenView} />
                    <NavButton 
                        label="บำรุงรักษา" 
                    ...
                    {(currentUser?.role === 'admin' || currentUser?.role === 'branch-admin') && (
                        <NavButton 
                            ...
                            isActive={currentView === 'closing-checklist'} 
                            onClick={() => onViewChange('closing-checklist')}
                ...
                <div className="flex items-center gap-4">
                    ...
                    {(currentUser.role === 'admin' || currentUser.role === 'branch-admin') && (
                        <div className="relative group">
                            <label className="relative inline-flex items-center cursor-pointer">
                              ...
                              <div className={`w-14 h-7 rounded-full peer peer-focus:outline-none peer-focus:ring-4 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-1 after:left-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-yellow-400 ${
                                  isKitchenView ? 'bg-gray-800 peer-focus:ring-gray-700' : 'bg-gray-200 peer-focus:ring-yellow-300'
                              }`}></div>
                              <span className={`ml-3 text-sm font-medium hidden lg:block transition-colors ${
                                  isKitchenView ? 'text-gray-300' : 'text-gray-900'
                              }`}>โหมดแก้ไข</span>
                            </label>
                        </div>
                    )}
                    ...
                    <div 
                        className={`flex items-center gap-2 border-l pl-4 cursor-pointer rounded-lg px-2 py-1 transition-all ${
                            isKitchenView ? 'border-gray-800 hover:bg-gray-900' : 'border-gray-200 hover:bg-gray-50'
                        }`}
                    ...
                    >
                        ...
                        <button className={`p-2 rounded-full transition-colors ${
                            isKitchenView ? 'text-gray-400 hover:bg-gray-800' : 'text-gray-500 hover:bg-gray-200'
                        }`}>
