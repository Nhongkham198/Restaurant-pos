...
export const EditCompletedOrderModal: React.FC<EditCompletedOrderModalProps> = ({ isOpen, order, onClose, onSave, menuItems }) => {
    ...
    const handleAddItem = (menuItem: MenuItem) => {
        if (menuItem.optionGroups?.some(g => g.required)) {
            Swal.fire('ไม่สามารถเพิ่มได้', 'รายการนี้จำเป็นต้องมีการตั้งค่าเพิ่มเติมซึ่งไม่สามารถทำได้ในหน้านี้', 'warning');
            return;
        }
    ...
    const filteredMenuItems = useMemo(() => {
        ...
        return menuItems.filter(item => 
            ...
            !editedItems.some(editedItem => editedItem.id === item.id) // Don't show items already in the order
        ).slice(0, 5); // Limit results for performance/UI
    }, [searchTerm, menuItems, editedItems]);
