...
export const CompletedOrderCard: React.FC<CompletedOrderCardProps> = ({ order, onSplitOrder, isEditMode, onEditOrder, onInitiateCashBill, isSelected, onToggleSelection }) => {
    ...
    const cardClasses = useMemo(() => {
        ...
        if (order.isDeleted) {
            base += "border-red-200 bg-red-50/50";
    ...
    return (
        <div className={cardClasses}>
            <header className={`p-4 ${order.isDeleted ? 'bg-red-50/70' : 'bg-gray-50'} flex justify-between items-center`} >
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                        <div className={`flex items-center flex-wrap gap-x-2 gap-y-1 ${order.isDeleted ? 'opacity-70' : ''}`}>
                            <p className={`font-bold text-xl text-teal-700 ${order.isDeleted ? 'line-through' : ''}`}>
                                <span className="text-gray-500">#</span>{String(order.orderNumber).padStart(3, '0')}
                            </p>
                            <p className={`font-semibold text-lg text-gray-800 truncate ${order.isDeleted ? 'line-through' : ''}`}>โต๊ะ {order.tableName} ({order.floor})</p>
                            {order.isDeleted && <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-800 font-bold border border-red-200">(ลบโดย: {order.deletedBy || 'Unknown'})</span>}
                        </div>
                        {order.customerName && (
                ...
                <div className="flex items-center gap-4 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <p className={`text-2xl font-bold text-gray-800 ${order.isDeleted ? 'line-through opacity-70' : ''}`}>{total.toLocaleString(undefined, { minimumFractionDigits: 2 })} ฿</p>
            ...
            {isExpanded && (
                <div className="p-4 border-t">
                    ...
                    <div className={`space-y-2 border-t pt-3 ${order.isDeleted ? 'opacity-50' : ''}`}>
                        <h4 className="font-semibold text-gray-700 mb-2">รายการอาหาร</h4>
                        {order.items.map(item => (
                            <div key={item.cartItemId} className={`text-base text-gray-700 py-1 ${order.isDeleted ? 'line-through' : ''}`}>
                    ...
                    <div className="mt-4 pt-4 border-t flex justify-end gap-3">
                        ...
                        {/* Only visible in edit mode and if not deleted */}
                        {isEditMode && !order.isDeleted && (
                            <>
