        ...
        <div className={cardClasses}>
            <header className="p-4 bg-red-50 flex justify-between items-center">
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                        <div className={`flex items-center flex-wrap gap-x-2 gap-y-1 ${order.isDeleted ? 'opacity-70' : ''}`}>
                            <p className={`font-bold text-xl text-red-700 ${order.isDeleted ? 'line-through' : ''}`}>
                ...
                <div className="flex items-center gap-4 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <p className={`text-2xl font-bold text-red-700 ${order.isDeleted ? 'line-through opacity-70' : ''}`}>{totalValue.toLocaleString(undefined, { minimumFractionDigits: 2 })} ฿</p>
            ...
            {isExpanded && (
                <div className="p-4 border-t">
                    <div className="mb-4 text-base bg-red-100 p-3 rounded-md border border-red-200">
                     ...
                     <div className={`space-y-2 ${order.isDeleted ? 'opacity-50' : ''}`}>
