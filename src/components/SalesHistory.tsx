...
export const SalesHistory: React.FC<SalesHistoryProps> = ({ completedOrders, cancelledOrders, printHistory, onReprint, onSplitOrder, isEditMode, onEditOrder, onInitiateCashBill, onDeleteHistory, currentUser }) => {
    ...
    const filteredCompletedOrders = useMemo(() => {
        ...
        // Soft delete logic: Only admin sees deleted items
    ...
    const handleDeleteClick = () => {
        ...
        const title = isAdmin ? 'ยืนยันการลบถาวร?' : 'ยืนยันการซ่อนรายการ?';
        const html = isAdmin
            ? `คุณเป็น Admin และกำลังจะลบ <b>${totalSelected}</b> รายการออกจากระบบอย่างถาวร<br/><br/><b class="text-red-600">การกระทำนี้ไม่สามารถย้อนกลับได้!</b>`
            : `คุณกำลังจะซ่อน <b>${totalSelected}</b> รายการที่เลือก<br/><br/>รายการเหล่านี้จะถูกซ่อนจากมุมมองของคุณ แต่ Admin ยังสามารถตรวจสอบได้`;
        const confirmButtonText = isAdmin ? 'ใช่, ลบถาวร!' : 'ใช่, ซ่อนเลย!';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                handleDeselectAll();
                // Success message is now handled in App.tsx
            }
        });
    };
