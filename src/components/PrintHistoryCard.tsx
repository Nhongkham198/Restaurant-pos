...
export const PrintHistoryCard: React.FC<PrintHistoryCardProps> = ({ entry, isEditMode, isSelected, onToggleSelection, onReprint }) => {
    ...
    return (
        <div className={cardClasses}>
            <header className={`p-4 ${entry.isDeleted ? 'bg-red-50/70' : 'bg-gray-50'} flex justify-between items-center`} >
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                        <div className={`flex items-baseline gap-2 ${entry.isDeleted ? 'opacity-75' : ''}`}>
                            <p className={`font-bold text-xl text-blue-700 ${entry.isDeleted ? 'line-through' : ''}`}>
                                <span className="text-gray-500">#</span>{String(entry.orderNumber).padStart(3, '0')}
                            </p>
                            <p className={`font-semibold text-lg text-gray-800 truncate ${entry.isDeleted ? 'line-through' : ''}`}>โต๊ะ {entry.tableName}</p>
                            ...
                            {entry.isDeleted && <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-800 font-bold border border-red-200">(ลบโดย: {entry.deletedBy || 'Unknown'})</span>}
                        </div>
            ...
            {isExpanded && (
                <div className="p-4 border-t">
                    <div className={`space-y-2 ${entry.isDeleted ? 'opacity-50' : ''}`}>
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-600 pl-2 space-y-1">
                           {entry.orderItemsPreview.map((item, index) => <li key={index} className={entry.isDeleted ? 'line-through' : ''}>{item}</li>)}
                        </ul>
                    </div>
                    {entry.errorMessage && (
